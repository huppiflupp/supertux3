...
class Boss(Entity):
    ...
    def __init__(self, x, y, assets):
        ...
        self.assets = assets
        self.walk = Animation(assets.boss["walk"], fps=4)
        self.hurt_img = assets.boss["hurt"][0]
        self.hp = 3
        self.invuln = 0.0
        self.defeated = False
        self.defeat_t = 1.6
    ...
    @property
    def speed(self):
        return 70.0 + (3 - self.hp) * 26.0     # wird mit jedem Treffer schneller
    ...
    def update(self, dt, level):
        ...
        self.throw_t -= dt
        if self.throw_t <= 0:
            self.throw_t = max(0.9, 1.8 - (3 - self.hp) * 0.3)
            px = level.player.cx
