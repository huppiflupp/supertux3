...
class Cat(_Walker):
    """Wüstenkatze – flinker Läufer, stampfbar."""
    stompable = True
    speed = 82.0

    def __init__(self, x, y, assets):
        super().__init__(x, y, 26, 14)
        self.walk = Animation(assets.cat, fps=8)
        self.flat = assets.cat[0]
        self.facing = -1
        self.squashed = False
        self.squash_t = 0.0
...
class Robot(_Walker):
    """Roboter – rollender Läufer, stampfbar (Großstadt-Welt)."""
    stompable = True
    speed = 64.0

    def __init__(self, x, y, assets):
        super().__init__(x, y, 22, 26)
        self.walk = Animation(assets.robot, fps=6)
        self.flat = assets.robot[0]
        self.facing = -1
        self.squashed = False
        self.squash_t = 0.0
